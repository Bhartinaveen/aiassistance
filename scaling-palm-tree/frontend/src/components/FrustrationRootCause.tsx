...
export default function FrustrationRootCause({ data }: { data: any[] }) {
  ...
  const frustrationKeywords = useMemo(() => {
    ...
    data.forEach((curr: any) => {
      const topic = curr.evaluation?.Root_Cause || curr.evaluation?.Bottleneck;
  ...
  return (
    <div className="h-80 w-full p-6 bg-indigo-950/20 shadow-[0_0_30px_rgba(79,70,229,0.15)] border border-indigo-500/20 rounded-2xl backdrop-blur-xl flex flex-col justify-center">
      ...
      {frustrationKeywords.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full gap-3">
          <p className="text-rose-200/30 text-sm font-bold">No frustration hotspots detected</p>
          <p className="text-rose-200/15 text-[10px] uppercase tracking-widest">Analyzed data shows no recurring topics</p>
        </div>
      ) : (
